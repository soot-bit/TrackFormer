...
class Event:
    ...
    def display(self):
        ...
        radii = [
            layer["radius"]
            for layer in self.detector.layers
            if self.detector.dimension == 2 and layer["shape"] == "cylinder"
        ]

        # Plot each cylindrical layer
        for radius in radii:
            circle = plt.Circle(
                (0, 0), radius, color="gray", fill=False, linestyle="--"
            )
            ax.add_patch(circle)

        # Plot the tracks
        ax.plot(
            self.hits.x.values[self.tracks],
            self.hits.y.values[self.tracks],
            color="k",
            linewidth=1,
            alpha=0.3,
            label="Truth Tracks",
        ...
        colors = cmap(
            non_noise_hits["particle_id"] / non_noise_hits["particle_id"].max()
        )  # Apply the colormap to your normalized particle IDs
        ax.scatter(
            non_noise_hits["x"], non_noise_hits["y"], color=colors, s=10, label="Hits"
        )
...
class EventGenerator:
    ...
    def generate_event(self):
        ...
        if isinstance(self.particle_gun, list):
            particles = pd.concat(
                [gun.generate_particles() for gun in self.particle_gun],
                ignore_index=True,
            )
        else:
    ...
    def _get_num_noise(
        ...
        if isinstance(noise, float):
            ...
            isinstance(noise, list) and len(noise) == 2 and isinstance(noise[0], float)
        ):
            noise = [int(noise[0] * num_hits), int(noise[1] * num_hits)]
        elif (
            isinstance(noise, list) and len(noise) == 3 and isinstance(noise[0], float)
        ):
            noise = [int(noise[0] * num_hits), int(noise[1] * num_hits), noise[2]]

        return int(self._generate_value(noise))

    def _generate_truth_tracks(
        self, particles: pd.DataFrame, hits: pd.DataFrame
    ) -> np.ndarray:
        """
        ...
        merged_hits["R"] = np.hypot(
            merged_hits["x"] - merged_hits["vx"], merged_hits["y"] - merged_hits["vy"]
        )
        sorted_hits = merged_hits.sort_values(by=["particle_id", "R"])

        # Get the edges of the tracks
        track_edges = np.stack(
            [sorted_hits.hit_id.values[:-1], sorted_hits.hit_id.values[1:]], axis=0
        )

        # Remove edges that don't have the same particle ID, just in case
        track_edges = track_edges[
            :,
            hits.particle_id.values[track_edges[0]]
            == hits.particle_id.values[track_edges[1]],
        ]

        return track_edges

    def _generate_value(
        self, value: Union[float, List[float], List[Union[float, str]]]
    ) -> float:
        """
